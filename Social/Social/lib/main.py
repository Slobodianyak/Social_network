...
# CREATE DATABASE
db = client['social']#mydatabase
# CREATE COLLECTION
users_collection = db['users']
note_collection = db['note']

print_all(users_collection)
get_multiple_data(users_collection)
print_all(users_collection)

print_all(note_collection)
...
#    for x in doc:
#        print(x)
#def show_user_info(collection):
#    for x in collection.find():
#        print(x)
##def add_friend(collection,id):
#def update_user_info(collection):
#    pass
#import pymongo
#client = pymongo.MongoClient('localhost', 27017)#path
#db = client['social']#mydatabase
#users_collection = db['users']
#note_collection = db['note']
...
#db1 = client['note']
#print_all(note_collection)
##sort_by(users_collection,"login")
